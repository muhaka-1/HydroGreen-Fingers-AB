...
// Route: GET / -> Serverar den interaktiva HTML-dashboarden
static esp_err_t root_get_handler(httpd_req_t *req) {
    httpd_resp_set_type(req, "text/html");
    return httpd_resp_send(req, WEB_DASHBOARD_HTML, HTTPD_RESP_USE_STRLEN);
}

// Route: GET /api/sensors -> Returnerar senaste sensordata som JSON
...
httpd_handle_t start_webserver(void) {
    ...
    ESP_LOGI(TAG, "Startar inbyggd HTTP-server på port %d (Placeholder för Emil)", config.server_port);
    
    // Serverkonfiguration och route-registrering
